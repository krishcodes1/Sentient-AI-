...
import { getOpenClawEmbedUrl, getOpenClawStatus } from "@/services/api";
...
export default function Gateway() {
  // Resolve embed URL from API (the backend can override per-environment),
  // falling back to the env constant so reverse-proxied deploys also work.
  const embedQuery = useQuery({
    queryKey: ["openclaw-embed-url"],
    queryFn: getOpenClawEmbedUrl,
  });
  ...
  const url = embedQuery.data?.url
    ? normalizeUrl(embedQuery.data.url)
    : normalizeUrl(OPENCLAW_BROWSER_URL);

  const notice = embedQuery.isError
    ? "Could not load URL from API — using the configured fallback."
    : !statusQuery.isLoading && statusQuery.data && !statusQuery.data.gateway_online
    ? "Gateway is unreachable. The Control UI may not load."
    : "";
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 min-w-0 bg-[var(--bg-primary)]">
      ...
      {notice ? (
        <p className="shrink-0 px-3 py-1.5 text-[11px] text-[var(--accent-warning)] bg-[rgba(251,191,36,0.08)] border-b border-[var(--claw-border)] inline-flex items-center gap-1.5">
          <AlertTriangle className="w-3 h-3" strokeWidth={2.5} aria-hidden="true" />
          {notice}
        </p>
      ) : null}
      ...
      <p className="shrink-0 px-3 py-2 text-[10px] leading-snug text-[var(--text-muted)] font-mono border-t border-[var(--claw-border)] bg-[var(--claw-panel)]">
        Full native OpenClaw dashboard — channels, WebChat, sessions, and config.
        If the frame is empty, your browser may block cross-origin embedding; use
        &quot;Open in new tab&quot;.
      </p>
    </div>
  );
